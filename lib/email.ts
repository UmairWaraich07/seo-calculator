...
export async function sendReportEmail(email: string, report: any) {
  ...
  // Generate HTML email string
  const emailHtml = generateReportEmailHTML(reportData, basicInfo);
...
/**
 * Generate HTML email string for the report using the preferred template
 */
function generateReportEmailHTML(report: any, basicInfo: any): string {
  ...
  // Calculate conversion rate (assuming 2.5% default)
  ...
  // Mock current rankings for display
  const currentRankings = {
    total: keywordData.length,
    top3: keywordData.filter((kw: any) => kw.currentRank && kw.currentRank <= 3)
      .length,
    top10: keywordData.filter(
      (kw: any) => kw.currentRank && kw.currentRank <= 10
    ).length,
    top50: keywordData.filter(
      (kw: any) => kw.currentRank && kw.currentRank <= 50
    ).length,
    top100: keywordData.filter(
      (kw: any) => kw.currentRank && kw.currentRank <= 100
    ).length,
  };

  // Mock competitor rankings
  const competitorRankings = [
    { name: "competitor1.com", top3: 12, top10: 28, top50: 45, top100: 67 },
    { name: "competitor2.com", top3: 8, top10: 22, top50: 38, top100: 52 },
    { name: "competitor3.com", top3: 6, top10: 18, top50: 32, top100: 48 },
  ];
  ...
  const previewText = `SEO Opportunity Report for ${
    basicInfo.businessUrl
  } - ${formatCurrency(potentialRevenue)} revenue potential discovered`;
...
<body>
  ...
  <div class="container">
    ...
    <div class="cta-section">
      ...
      }/schedule-call?ref=email&scope=${
    basicInfo.analysisScope
  }" class="button">Schedule Your Free Strategy Call</a>
    </div>
    ...
    <div class="footer">
      ...
      <p style="font-weight: 700; color: #374151; margin-bottom: 8px;">© ${new Date().getFullYear()} Your Agency Name. All rights reserved.</p>
